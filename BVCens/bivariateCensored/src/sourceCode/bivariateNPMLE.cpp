...
void SEXPMat2pmass_info(SEXP r_mat, std::vector< std::vector<int> > &cliqMat ){
    std::vector<int> dims = getSEXP_MatDims(r_mat);
    //ASSUMING ROWS = NUMBER OF OBSERVATIONS: CAN'T RECALL EXACTLY
    int N  = dims[0];
    int K  = dims[1];
    cliqMat.resize(K);
    int k1, k2;
    for(int i = 0; i < K; i++){
        k1 = 0;
        for(int j = 0; j < N; j++){
            if(INTEGER(r_mat)[i * N + j] == 1) {k1++;}
        }
        cliqMat[i].resize(k1);
        k2 = 0;
        for(int j = 0; j < N; j++){
            if(INTEGER(r_mat)[i * N + j] == 1) {
                cliqMat[i][k2] = j;
                k2++;
            }
        }
...
void SEXPMat2pobs_info(SEXP r_mat, std::vector< std::vector<int> > cliqMat ){
    std::vector<int> dims = getSEXP_MatDims(r_mat);
    //ASSUMING ROWS = NUMBER OF OBSERVATIONS: CAN'T RECALL EXACTLY
    int N  = dims[0];
    int K  = dims[1];
    ...
    for(int i = 0; i < N; i++){
        k1 = 0;
        for(int j = 0; j < K; j++){
            if(INTEGER(r_mat)[i * K + j] == 1) {k1++;}
        }
        cliqMat[i].resize(k1);
        k2 = 0;
        for(int j = 0; j < K; j++){
            if(INTEGER(r_mat)[i * K + j] == 1) {
                cliqMat[i][k2] = j;
                k2++;
            }
        }
...
bvcen::bvcen(SEXP cliqMat){
    std::vector<int> dims = getSEXP_MatDims(cliqMat);
    int n_obs = dims[0];
    int n_mi = dims[1];
    p_obs.resize(n_obs);
    SEXPMat2pmass_info(cliqMat, pmass_in_ob);
...
SEXP optCliq(SEXP cliqMat, SEXP R_tol, SEXP R_innerLoops, SEXP R_outerLoops){
